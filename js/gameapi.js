		...
		return new Promise(function(resolve, reject) {
			if (M.provider === "dfp") {
				...
				faZepto.each(rules, function(key, ruleName) {
					...
					adTagUrls.push(M.getAdTagUrl(variant, perRulePriceMode, waterfallNumber++, 0));
					if (!detection.is.iframe && key == 0) {
						adTagUrls.push(M.getAdTagUrl("", "", waterfallNumber++, 2));
					}
					adTagUrls.push(M.getAdTagUrl(variant, perRulePriceMode, waterfallNumber++, 1));
					adTagUrls.push(M.getAdTagUrl(variant, perRulePriceMode, waterfallNumber++, 12));
				});
